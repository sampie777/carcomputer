...
void content_about_cruise_control(const State *state, SH1106Config *display) {
    ...
    sprintf(buffer, "Kp: %.6f", state->speed_control.cruise_control.pidKp);
    sh1106_draw_string(display, offset_x, offset_y, FONT_SMALL, FONT_WHITE, buffer);
    offset_y += 9;
    sprintf(buffer, "Ki:  %.6f", state->speed_control.cruise_control.pidKi);
    sh1106_draw_string(display, offset_x, offset_y, FONT_SMALL, FONT_WHITE, buffer);
    offset_y += 9;
    sprintf(buffer, "Kd: %.6f", state->speed_control.cruise_control.pidKd);
    sh1106_draw_string(display, offset_x, offset_y, FONT_SMALL, FONT_WHITE, buffer);
    offset_y += 10;
